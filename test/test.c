...
/* Story 20: As a user,I want the calculator to take a single
 *   character off the line, so it doesn't print out an “invalid
 *   command” message every time I enter a command
 */
/* v1 write a test for whitespace */
void test_is_whitespace(void) {
  for (char op = ' '; op <= '~'; op++) {
    if (' ' == op || '\t' == op || '\n' == op) {
      CU_ASSERT_TRUE(is_whitespace(op));
    } else {
      CU_ASSERT_FALSE(is_whitespace(op));
    }
  }
}
  ...
  return
    (
     ...
     && NULL != CU_add_test(pSuite, "Testing: valid_operator(char)",
			    test_valid_operator)
     && NULL != CU_add_test(pSuite, "Testing: is_whitespace(char)",
			    test_is_whitespace)
